...
def build_donut_chart(segments, output_path: str, width=400, height=400):
    ...
    if not segments or total_seconds == 0:
        ...
        painter.drawText(
            QRectF(cx - 100, cy - 20, 200, 40), Qt.AlignmentFlag.AlignCenter, "No Data"
        )
    else:
        current_angle = 90.0
        for s in segments:
        ...
        painter.drawText(
            QRectF(cx - r_inner, cy - 20, 2.0 * r_inner, 25),
        ...
        painter.drawText(
            QRectF(cx - r_inner, cy + 8, 2.0 * r_inner, 20),
            Qt.AlignmentFlag.AlignCenter,
            "Tracked",
        )
    ...
    if not data or chart_w <= 0 or chart_h <= 0:
        ...
        for i in range(grid_lines + 1):
            ...
            if i > 0:
                ...
                painter.setPen(pen)
                painter.drawLine(
                    QPointF(padding_left, gy), QPointF(width - padding_right, gy)
                )

            painter.setPen(QColor("#64748B"))
            label = f"{int(val)}h" if y_max >= 1 else f"{val:.1f}h"
            painter.drawText(
                QRectF(5, gy - 8, padding_left - 10, 16),
        ...
        painter.drawLine(
            QPointF(padding_left, height - padding_bottom),
            QPointF(width - padding_right, height - padding_bottom),
        )
