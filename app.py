...
from utils.icon_utils import pil_to_pixmap, get_native_icon_pixmap
...
# pil_to_pixmap and get_native_icon_pixmap moved to utils/icon_utils.py
...
# HeaderBar moved to widgets/header_bar.py
...
class TrueHourApp(QMainWindow):
    ...
    def _show_about_dialog(self):
        ...
        dialog = AboutDialog(theme_style=self.theme_style, parent=self)
        dialog.exec()
    ...
    def _show_categories_dialog(self):
        ...
        dialog = CategoriesDialog(self.tracker, theme_style=self.theme_style, parent=self)

        def _on_categories_changed():
            self._last_app_state_hash = None
            self._refresh_app_list()

        dialog.categories_changed.connect(_on_categories_changed)
        dialog.exec()
    ...
    def _show_report(self, report, is_new=True, is_live=False):
        ...
        dialog.export_requested.connect(self._export)
        dialog.refresh_requested.connect(self._show_live_report)
        dialog.exec()
